...
if(!isset($_SESSION['user_id'])){
    $_SESSION['login_errors'] = ['Please log in to access your dashboard.'];
...
if ($database_handler && $pdo = $database_handler->getConnection()) { 
    try {
        ...
        $stmt_comments = $pdo->prepare("SELECT COUNT(*) FROM comments WHERE user_id = ?"); // Assuming a 'comments' table.
        $stmt_comments->execute([$userId]);
        $user_comment_count = (int)$stmt_comments->fetchColumn();


        $stmt_notifications = $pdo->prepare("SELECT COUNT(*) FROM notifications WHERE user_id = ? AND is_read = 0"); // Assuming 'notifications' table.
...
<div class="page-container dashboard-container"> 
    ...
    <p class="dashboard-intro">Here you can manage your articles, profile, and account settings for the blog.</p> <!-- Updated intro text -->

    <!-- Overview Cards Section (Stats) -->
    <div class="dashboard-overview">
        <div class="overview-card">
            <span class="overview-card-icon">📄</span> <!-- Icon for articles -->
            <span class="overview-card-value"><?php echo $user_article_count; ?></span>
            <span class="overview-card-label">Your Articles</span>
        </div>
        <div class="overview-card">
            <span class="overview-card-icon">💬</span> <!-- Icon for comments -->
            <span class="overview-card-value"><?php echo $user_comment_count; ?></span>
            <span class="overview-card-label">Your Comments</span>
        </div>
        <div class="overview-card">
            <span class="overview-card-icon">🔔</span> <!-- Icon for notifications -->
